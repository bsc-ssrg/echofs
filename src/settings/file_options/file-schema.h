...
#ifndef __FILE_SCHEMA_H__
#define __FILE_SCHEMA_H__
...
struct typed_value : public option_type {
    ...
    typed_value(const T default_value) 
    ...
    typed_value(const ParserFun<T>& converter)
        : m_has_default_value(false),
...
template <typename T>
OptionSchemaType declare_option(const std::string& name, bool mandatory) {
    return {name, {mandatory, type<T>()}};
}

template <typename T>
OptionSchemaType declare_option(const std::string& name, bool mandatory, const T& default_value) {
    return {name, {mandatory, type<T>(default_value)}};
}

template <typename T>
OptionSchemaType declare_option(const std::string& name, bool mandatory, const ParserFun<T>& parser) {
    return {name, {mandatory, type<T>(parser)}};
}

template <typename T>
OptionSchemaType declare_option(const std::string& name, bool mandatory, const T& default_value, const ParserFun<T>& parser) {
    return {name, {mandatory, type<T>(default_value, parser)}};
}

list_schema declare_list(const std::initializer_list<OptionSchemaType>&& args) {
    return {args};
}

group_schema declare_group(const std::initializer_list<OptionSchemaType>&& args) {
    return {args};
}

using SectionSchemaType = std::pair<const std::string, section_schema>;

SectionSchemaType declare_section(const std::string& name, bool mandatory, const group_schema&& schema) {
    return {name, {mandatory, {schema}}};
}

SectionSchemaType declare_section(const std::string& name, bool mandatory, const list_schema&& schema) {
    return {name, {mandatory, {schema}}};
}

/*! Declares a file_schema as a list of section_schema definitions */
file_schema declare_file(const std::initializer_list<SectionSchemaType>&& args) {
    return {args};
}

} // namespace file_options

#endif /* __FILE_SCHEMA_H__ */
